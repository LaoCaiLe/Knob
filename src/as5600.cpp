...
void AS_5600::begin()
{
    AS5600_Wire.begin(AS5600_SDA_PIN,AS5600_SCL_PIN,1000000);
}
...
/* register.
/*******************************************************/
word AS_5600::setMaxAngle(word newMaxAngle)
{
  word retVal;
  if(newMaxAngle == -1)
  {
...
/* magnet.  
/*******************************************************/
word AS_5600::setStartPosition(word startAngle)
{
  if(startAngle == -1)
  {
...
/* magnet.  
/*******************************************************/
word AS_5600::setEndPosition(word endAngle)
{
  if(endAngle == -1)
...
/*******************************************************/
int AS_5600::burnMaxAngleAndConfig()
{
  int retVal = 1;
  _maxAngle  = getMaxAngle();
...
/*******************************************************/
int AS_5600::readOneByte(int in_adr)
{
  int retVal = -1;
  AS5600_Wire.beginTransmission(_ams5600_Address);
  AS5600_Wire.write(in_adr);
  AS5600_Wire.endTransmission();
  AS5600_Wire.requestFrom(_ams5600_Address, 1);
  while(AS5600_Wire.available() == 0);
  retVal = AS5600_Wire.read();
...
/*******************************************************/
word AS_5600::readTwoBytes(int in_adr_hi, int in_adr_lo)
{
  word retVal = -1;
 
  /* Read Low Byte */
  AS5600_Wire.beginTransmission(_ams5600_Address);
  AS5600_Wire.write(in_adr_lo);
  AS5600_Wire.endTransmission();
  AS5600_Wire.requestFrom(_ams5600_Address, 1);
  while(AS5600_Wire.available() == 0);
  int low = AS5600_Wire.read();
 
  /* Read High Byte */  
  AS5600_Wire.beginTransmission(_ams5600_Address);
  AS5600_Wire.write(in_adr_hi);
  AS5600_Wire.endTransmission();
  AS5600_Wire.requestFrom(_ams5600_Address, 1);
  
  while(AS5600_Wire.available() == 0);
  
  word high = AS5600_Wire.read();
...
/*******************************************************/
void AS_5600::writeOneByte(int adr_in, int dat_in)
{
  AS5600_Wire.beginTransmission(_ams5600_Address);
  AS5600_Wire.write(adr_in); 
  AS5600_Wire.write(dat_in);
  AS5600_Wire.endTransmission();
}

float AS_5600::convertScaledAngleToDegrees()
{
  word startPos = getStartPosition();
  word endPos = getEndPosition();
  word maxAngle = getMaxAngle();
  word newAngle = getScaledAngle();
  float multipler = 0;
...
float AS_5600::convertRawAngleToDegrees()
{
